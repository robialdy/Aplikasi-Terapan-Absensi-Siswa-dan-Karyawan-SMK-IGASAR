                ...
                <form action="{{ route('kelas.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control" id="nama_kelas" placeholder="Masukan Tanggal mulai" name="nama_kelas" value="{{ old('nama_kelas') }}">
                            @error('nama_kelas')
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="walikelas" id="walikelas" class="form-select">
                                ...
                                @foreach ($walikelas as $wk)
                                    <option value="{{ $wk->id }}" @if (old('walikelas') == $wk->id)  @endif >{{ $wk->nama_lengkap }} - {{ $wk->bidang }}</option>
                                @endforeach
                            </select>
                            @error('walikelas')
                            <small class="text-danger">{{ $message }}</small>
